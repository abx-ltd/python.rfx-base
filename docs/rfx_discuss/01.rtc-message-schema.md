...
- A dedicated acknowledgement message type is not used. Acknowledgements are represented as replies via `discuss.message.reply` with `reply_kind: "acknowledgement"` (and edited/deleted through `discuss.reply.edit` / `discuss.reply.delete`).
...
sequenceDiagram
    participant C as Client
    participant S as DiscussServer
    C->>S: discuss.message.reply reply_of reply_kind
    S->>C: discuss.confirm.sent
```
