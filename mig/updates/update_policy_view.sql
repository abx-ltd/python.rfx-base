...
FROM rfx_user.profile
    JOIN rfx_user.profile_role AS pro_rol ON profile._id = pro_rol.profile_id
...
SELECT
    ...
    'user-profile'::character varying(255) AS dom,
    'invitation'::character varying(255) AS res,
    invitation.organization_id::character varying(255) AS rid,
    NULL::character varying(255) AS act,
    NULL::character varying(255) AS cqrs,
    NULL::character varying(255) AS meta,
    NULL::timestamp without time zone AS _deleted
...
SELECT
    ...
    NULL::timestamp without time zone AS _deleted
FROM rfx_user."group"
WHERE "group".resource_id IS NOT NULL
  AND "group".resource::text = 'organization'::text
UNION ALL
SELECT
    uuid_generate_v4() AS _id,
