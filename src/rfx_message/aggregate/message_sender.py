...
class MessageSenderMixin:
    ...
    @action(
        "sender-added",
        resources=("message", "message_sender", "message_box", "message_box_user"),
    )
    async def add_sender(self, *, message_id, sender_id, recipients=None):
        """Add sender to a message using a shared member box."""

        # Check if box not exist so create new box
        member_box = await self.get_or_create_member_box(sender_id, recipients)

        sender_data = {
            "message_id": message_id,
            "sender_id": sender_id,
            "box_id": member_box._id,
            "direction": "OUTBOUND",
        }
