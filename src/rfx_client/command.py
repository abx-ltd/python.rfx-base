...
class AddWorkPackageToEstimator(Command):
    ...
    class Meta:
        key = "add-workpackage-to-estimator"
        resources = ("project",)
        tags = ["project", "workpackage"]
        auth_required = True
    ...
    async def _process(self, agg, stm, payload):
        ...
        await agg.add_work_package_to_estimator(
...
class CalculateTotalCreditCost(Command):
    """Calculate Total Credit Cost - Calculates the total credit cost based on Work Packages"""

    class Meta:
        key = "calculate-credit"
        resources = ("project",)
        tags = ["project", "credit"]
        auth_required = True
        description = "Calculate total credit cost for current estimator"

    async def _process(self, agg, stm, payload):
        """Calculate total credit cost"""
        credits_breakdown = await agg.calculate_credit_cost()
        yield agg.create_response(credits_breakdown, _type="credit-breakdown")
...
class UpdateWorkPackageQuantity(Command):
    ...
    class Meta:
        key = "update-workpackage-from-estimator"
        resources = ("project",)
        tags = ["project", "workpackage"]
        auth_required = True
...
class RemoveWorkPackageFromEstimator(Command):
    ...
    class Meta:
        key = "remove-workpackage-from-estimator"
        resources = ("project",)
        tags = ["project", "workpackage"]
        auth_required = True
...
class CreateWorkPackage(Command):
    ...
    class Meta:
        key = "create-workpackage"
        resources = ("work-package",)
        tags = ["workpackage", "create"]
        new_resource = True
        auth_required = True
...
class UpdateWorkPackage(Command):
    ...
    class Meta:
        key = "update-workpackage"
        resources = ("work-package",)
        tags = ["workpackage", "update"]
        auth_required = True
