...
class LinearProvider(PMService):
    ...
    async def update_ticket(self, payload: UpdateTicketPayload) -> UpdateTicketResponse:
        ...
        try:
            logger.info(
                f"[Linear] Updating issue {payload.external_id} with data: {update_data}"
            )
            result = await self._client.update_issue(
                issue_id=payload.external_id, data=update_data
            )
    ...
    async def create_project(
        self, payload: CreateProjectPayload
    ) -> CreateProjectResponse:
        linear_data = {
            "name": payload.name,
    ...
    async def update_project(
        self, payload: UpdateProjectPayload
    ) -> UpdateProjectResponse:
        linear_data = {}
    ...
    async def delete_project(self, project_id: str, permanently: bool = False) -> bool:
        try:
            logger.info(
                f"[Linear] Deleting project: {project_id} (permanently={permanently})"
            )

            # Call LinearIntegration client
            result = await self._client.delete_project(
                project_id=project_id, permanently=permanently
            )
    ...
    async def create_project_milestone(
        self, payload: CreateProjectMilestonePayload
    ) -> CreateProjectMilestoneResponse:
        linear_data = {
            "name": payload.name,
    ...
    async def update_project_milestone(
        self, payload: UpdateProjectMilestonePayload
    ) -> UpdateProjectMilestoneResponse:
        linear_data = {}
        ...
        try:
            ...
            result = await self._client.update_project_milestone(
                payload.external_id, linear_data
            )
    ...
    async def create_comment(
        ...
        comment_data = {"body": payload.body, "issueId": payload.target_id}
    ...
    async def update_comment(
        self, payload: UpdateCommentPayload
    ) -> UpdateCommentResponse:
        """
        ...
        try:
            ...
            result = await self._client.update_comment(
                comment_id=payload.external_id, body=payload.body
            )
    ...
    async def verify_webhook_signature(self, request: Request) -> bool:
        ...
        if not webhook_secret:
            logger.error("[Linear] No webhook secret configured")
            raise HTTPException(status_code=500, detail="Webhook secret not configured")

        signature_hash = x_webhook_signature
        if x_webhook_signature.startswith("sha256="):
            signature_hash = x_webhook_signature[7:]

        try:
            payload_dict = json.loads(body_bytes)
        except json.JSONDecodeError:
            logger.error("[Linear] Failed to decode JSON body")
            raise HTTPException(status_code=400, detail="Invalid JSON body")

        payload_str_to_hash = json.dumps(payload_dict, sort_keys=True)

        expected_signature = hmac.new(
            webhook_secret.encode("utf-8"),
            payload_str_to_hash.encode("utf-8"),
            hashlib.sha256,
        ).hexdigest()

        if not hmac.compare_digest(signature_hash, expected_signature):
            logger.error("[Linear] Invalid webhook signature")
            logger.error(f"[Linear] Expected (tôi tính): {expected_signature}")
            logger.error(f"[Linear] Got (uapi gửi): {signature_hash}")
    ...
    async def parse_webhook_payload(self, request: Request) -> Dict[str, Any]:
        ...
        logger.info("[Linear] Received webhook event")
        logger.info(f"[Linear] Event type: {event_type}")
        logger.info(f"[Linear] Action: {event.get('action')}")

        if not event_type:
            logger.error("[Linear] Missing event type in payload")
            logger.error(f"[Linear] Payload keys: {list(payload.keys())}")
            logger.error(f"[Linear] Event keys: {list(event.keys())}")

        return {"event_type": event_type, "event_data": event, "raw_payload": payload}
    ...
    async def _handle_project_webhook(
        self, action: str, data: Dict[str, Any]
    ) -> WebhookResponse:
