...
project_estimate_summary_view = PGView(
    ...
    WITH project_credits AS (
         SELECT pwp.project_id,
           ...
           FROM {SCHEMA}.project_work_package pwp
             JOIN {SCHEMA}.work_package_work_item wpwi ON pwp.work_package_id = wpwi.work_package_id AND wpwi._deleted IS NULL
         ...
         SELECT p_1._id AS project_id,
           ...
           FROM {SCHEMA}.project p_1
             LEFT JOIN {SCHEMA}.promotion pr ON pr.code::text = p_1.referral_code_used::text AND pr._deleted IS NULL AND pr.valid_from <= now() AND pr.valid_until >= now() AND pr.current_uses < pr.max_uses
        )
        SELECT p._id,
            p._created,
            p._updated,
...
document_view = PGView(
    ...
    COALESCE(( SELECT json_agg(jsonb_build_object('id', part._id, 'name', COALESCE(part.preferred_name, concat(part.name__given, ' ', part.name__family)::character varying), 'avatar', part.picture_id, 'role', dp.role)) AS json_agg
           FROM {SCHEMA}.document_participant dp
             JOIN {USER_SCHEMA}.profile part ON part._id = dp.participant_id
   ...
   FROM {SCHEMA}.document d
     LEFT JOIN "{MEDIA_SCHEMA}"."media-entry" me ON d.media_entry_id = me._id
     LEFT JOIN {USER_SCHEMA}.profile creator ON d._creator = creator._id
