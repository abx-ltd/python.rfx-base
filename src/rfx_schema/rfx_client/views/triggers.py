...
BEGIN
    ...
    IF NEW.status = 'DONE'::{SCHEMA}.workpackageenum
       AND (OLD.status IS DISTINCT FROM 'DONE'::{SCHEMA}.workpackageenum) THEN
        NEW.date_complete := NOW();

    -- (Tuỳ chọn) Nếu chuyển từ DONE ngược lại TODO -> Xóa ngày hoàn thành
    ELSIF NEW.status != 'DONE'::{SCHEMA}.workpackageenum THEN
...
trg_pwp_auto_date_complete = PGTrigger(
    ...
    on_entity=f"{SCHEMA}.project_work_package",
    is_constraint=False,
    definition=f"""
        BEFORE UPDATE ON {SCHEMA}.project_work_package
        FOR EACH ROW
        EXECUTE FUNCTION {SCHEMA}.fn_update_pwp_date_complete()
    """,
)
...
trg_auto_update_credit_balance = PGTrigger(
    ...
    is_constraint=False,
    definition=f"""
        AFTER UPDATE OF status ON {SCHEMA}.project_work_package
        FOR EACH ROW
        EXECUTE FUNCTION {SCHEMA}.fn_update_org_credit_balance()
    """,
)
