...
FROM {config.RFX_CLIENT_SCHEMA}.supplier_service ss
JOIN {config.RFX_CLIENT_SCHEMA}.supplier sup ON ss.supplier_id = sup._id
JOIN {config.RFX_CLIENT_SCHEMA}.service_category sc ON ss.service_id = sc._id;
    """,
)
