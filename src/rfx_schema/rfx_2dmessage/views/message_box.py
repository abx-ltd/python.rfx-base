...
WITH thread_agg AS (
        ...
        UNION ALL
         SELECT s._id,
           ...
           FROM {config.RFX_2DMESSAGE_SCHEMA}.message_sender s
             JOIN {config.RFX_2DMESSAGE_SCHEMA}.message m ON m._id = s.message_id
             LEFT JOIN {config.RFX_2DMESSAGE_SCHEMA}.message_recipient r ON r.message_id = m._id AND r._deleted IS NULL
             LEFT JOIN thread_agg ta ON ta.thread_id = m.thread_id
             LEFT JOIN {config.RFX_2DMESSAGE_SCHEMA}.message_box mb ON mb._id = s.box_id AND mb._deleted IS NULL
             LEFT JOIN {config.RFX_2DMESSAGE_SCHEMA}.message_tag mt ON mt.resource::text = 'message_sender'::text AND mt.resource_id = s._id AND mt._deleted IS NULL
             LEFT JOIN {config.RFX_2DMESSAGE_SCHEMA}.tag t ON t._id = mt.tag_id AND t._deleted IS NULL
             LEFT JOIN {config.RFX_USER_SCHEMA}.profile sp ON sp._id = s.sender_id AND sp._deleted IS NULL
             LEFT JOIN {config.RFX_USER_SCHEMA}.profile rp ON rp._id = r.recipient_id AND rp._deleted IS NULL
         ...
         SELECT b._id,
            ...
            b.tags,
                CASE
                    WHEN b.direction = 'OUTBOUND'::{config.RFX_2DMESSAGE_SCHEMA}.directiontypeenum THEN row_number() OVER (PARTITION BY b.thread_id, b.box_key, b.target_profile_id ORDER BY b._created DESC)
                    ELSE 1::bigint
                END AS rn
           FROM base b
        )
 SELECT ranked._id,
    ...
    ranked.rn
   FROM ranked
  WHERE ranked.direction = 'OUTBOUND'::{config.RFX_2DMESSAGE_SCHEMA}.directiontypeenum AND ranked.rn = 1 OR ranked.direction = 'INBOUND'::{config.RFX_2DMESSAGE_SCHEMA}.directiontypeenum;
    """,
)
